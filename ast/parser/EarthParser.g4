...
earthFile: NL*? version? NL+? stmts? NL*? targets? NL* EOF;

targets: targetOrUserCommand+;
targetOrUserCommand: target | userCommand;
target: targetHeader NL+? (INDENT stmts NL+? DEDENT)?;
targetHeader: NL Target;
userCommand: userCommandHeader NL+? (INDENT stmts NL+? DEDENT)?;
userCommandHeader: NL UserCommand;
...
envStmt: ENV envArgKey EQUALS? envArgValue?;
argStmt: ARG optionalFlag envArgKey (EQUALS envArgValue?)?;
